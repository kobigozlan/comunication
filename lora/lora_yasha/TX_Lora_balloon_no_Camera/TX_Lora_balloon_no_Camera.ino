...
int idDHT11pin = 19; //Digital pin for comunications
int idDHT11intNumber = 19; //interrupt number (must be the one that use the previus defined pin (see table above)
...
void loop()
{
  ...
  bat = analogRead(A7);//ReadA7 which is internaly connected to the battery through resistor bridge
//  bat=bat*2*3.3/1024;
  rssi = rf95.lastRssi();
  snr = rf95.lastSNR();
  ...
  delay(10);
  rf95.waitPacketSent();
  Serial.println("sent seccessfully");
  t2 = millis();
