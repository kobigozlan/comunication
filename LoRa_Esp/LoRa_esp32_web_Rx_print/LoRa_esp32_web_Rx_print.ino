...
void setfreqserver(char* var) {
  server.on(var, HTTP_GET, [SetLora,var, frequency](AsyncWebServerRequest * request) {
...
void setbwserver(char* var) {
  server.on(var, HTTP_GET, [SetLora,var, bw](AsyncWebServerRequest * request) {
    bw = atol(var + 3) * 1000;
...
void setsfserver(char* var) {
  server.on(var, HTTP_GET, [SetLora,var, sf](AsyncWebServerRequest * request) {
    sf = atoi(var + 3);
...
void setcrserver(char* var) {
  server.on(var, HTTP_GET, [SetLora,var, CodingRate](AsyncWebServerRequest * request) {
...
//    Serial.println(WiFi.softAPIP());
  ...
  server.on("/", HTTP_GET, [UpdatePage](AsyncWebServerRequest * request) {
    request->send(SPIFFS, "/esp32Lora.html", String(), false, processor);
  });
...
void loop() {
  ...
  if (packetSize) {
    ...
    while (LoRa.available()) {
      LoraSend += (char)LoRa.read();
    }
    LoraSend += (" rssi: " + String(LoRa.packetRssi()));
