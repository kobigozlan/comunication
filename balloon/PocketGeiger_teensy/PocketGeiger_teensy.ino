...
int signPin = 3; //Radiation Pulse (Yellow)
int noisePin = 2; //Vibration Noise Pulse (White)
//VCC 5V (Red)
//GND (Blue)
////////////////////////////////
...
int index_count = 0; //Number of loops
...
void setup()
{
  Wire.begin(0xAA);                // join i2c bus with address #8
...
void loop()
{
  ...
  { //Deactivate Noise Pulse counting for a while
    nON = 1;
    noiseCount++;
    Serial.println("nnnnnnnnnnnnnnnnoooooooooooooiiiiiiiiiiisssssssssseeeeeeeee");
  } else if (noise == 0 && nON == 1) {
    nON = 0;
  }

  //Output readings to serial port, after 10000 loops
  if (index_count == 20000) //About 160-170 msec in Arduino Nano(ATmega328)
  {
    //Get current time
    currTime = millis();
...
void requestEvent() {
  byte sendVal[4];
  memcpy(sendVal,&cpmsend,4);
  Wire.write(sendVal[0]);
  Wire.write(sendVal[1]);
  Wire.write(sendVal[2]);
  Wire.write(sendVal[3]);
  
    Serial.println(sendVal[3],HEX); 
    Serial.println(sendVal[2],HEX); 
    Serial.println(sendVal[1],HEX); 
    Serial.println(sendVal[0],HEX);
    Serial.println(cpmsend);
    Serial.println();
}
